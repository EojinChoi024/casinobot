...
async def on_message(message):
    if message.content == "!돈받기":
        ...
        if ID in idA: #만약 등록된 ID라면
            if TIME - timeA[idA.index(ID)] < 600: #1시간이 안 지났을 때
                embed = discord.Embed(title='', description='10분마다 받을 수 있습니다.', color=0xFF0000)
                await message.channel.send(embed=embed)
                raise ValueError #탈출
            elif TIME - timeA[idA.index(ID)] >= 600: #1시간이 지났을 때
...
client.run("NzAwNjI0NTE4NTY1MzMxMDI0.Xpv1oA.klTNCHcL-Ixy9myL3dkWiRHV-_A")
